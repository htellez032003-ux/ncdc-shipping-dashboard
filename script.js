...
// Normalize CSV row to internal order object
function normalizeOrderRow(row) {
  const startDate = row["START DATE"] || "";
  const cancelDate = row["CANCEL DATE"] || "";
  
  // Calculate ready date (use start date as primary)
  ...
  return {
    ...
    loadType: loadType,
    priority: priority,
    isSPS: carrier.includes("UPS") || carrier.includes("FXG") || carrier.includes("FEDEX"),
    // User-editable fields (preserved on merge)
    timeBlock: "",
    assignedTruckId: null
  };
}

// Process and merge orders from CSV
function processOrdersFromCSV(parsedRows) {
  // Create map of existing orders using PO+BOL as unique key
  ...
  parsedRows.forEach(row => {
    ...
    if (existing.has(key)) {
      // Merge: update CSV fields but preserve user-created data
      const current = existing.get(key);
      
      // Preserve user-created fields
      const preserved = {
      ...
      // Merge normalized data with preserved fields
      Object.assign(current, normalized, preserved);
    } else {
      // New order - add to state
...
function applyQuickFilter(type) {
  ...
  if (type === "high") {
    ...
    // Filter orders without assigned truck
...
function saveTruckload() {
  ...
  // Update orders with assigned truck ID
...
function buildAutoProposals(ordersToRoute) {
  ...
  for (const [key, orders] of byGroup) {
    ...
    while (remaining.length > 0) {
      ...
      const date = earliestDate(chunk.map(o => o.readyDate || o.startDate));
...
function confirmAutoRoute() {
  ...
  toCreate.forEach(p => {
    ...
    const newTruck = {
      ...
      masterBol: mostCommon(p.orders.map(o => o.masterBol)),
      cartons: p.cartons,
      units: p.units,
      weight: p.orders.reduce((s, o) => s + (o.weight || 0), 0),
      stagedLocationSL: "",
      stagedLocationDD: "",
      status: "Not Started",
      departed: false,
